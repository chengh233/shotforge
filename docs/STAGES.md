...
每个阶段也有统一入口 `bash run.sh <stage> <project> [args]`。

### ① 视频 I2V（Colab GPU）
```bash
bash run.sh serve                                  # 起 ComfyUI（等 "up on :8188"）
bash run.sh video projects/example --shot s1       # 先单镜验证
bash run.sh video projects/example                 # 全部镜头
```
**看产物**（Colab cell 里）：
```python
...
### ② 配音（Mac）
```bash
bash run.sh dub projects/example --voice zh-CN-XiaoxiaoNeural    # 每句台词 → out/audio/<id>.mp3
```
**看产物**：`afplay projects/example/out/audio/s1.mp3`（Mac 播放）

### ③ 字幕（Mac）
```bash
bash run.sh subs projects/example                  # → out/<片名>.srt（按各镜时长对齐）
```
...
### ⑤ 后期合成（Mac）
```bash
bash run.sh post projects/example                          # 拼接 + 配音 + 烧字幕
bash run.sh post projects/example --music bgm.mp3          # 再加背景音乐（自动压低音量混音）
```
**看产物**：`open projects/example/out/夏风_final.mp4`（Mac 打开）

> 只想要无声拼接：`bash run.sh stitch projects/example` → `out/<片名>_full.mp4`。
...
- **下载用 aria2 快下**（`comfyui_setup.sh` 已用），缩短「GPU 空转等下载」的窗口。
